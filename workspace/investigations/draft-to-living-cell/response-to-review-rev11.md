...
| **Autopoiesis / EMERGE** | "The membrane is produced AND maintained from the inside — the paper's individual criterion." | **Genuinely emergent from local physics (Protocell v2, built + measured)**: production depends on a local interior precursor a closed membrane confines and an open one bleeds — **no `binary_fill_holes` observer in the update loop**. Closure persists ~2451 steps; precursor knockout collapses ~26× faster; a wedge puncture starves the membrane locally and never self-heals (a real local prediction). v1 relabeled as the specification-level realization. The open-system version (v2-open) is now **built and measured**, with an honest **negative** result: a local external drive does NOT reach steady self-maintenance — it destabilizes the ring into faster collapse (~591 vs ~2451 steps), and an ~80-point sweep found no local drive reaching a bounded steady state, because the metastability is **geometric** (secreted precursor pools in the shrinking pocket), not budget exhaustion. So EMERGE is **genuinely-emergent-but-metastable** — an honest measure of how hard steady emergent closure is. |
| **Chemotaxis (flagship frontier)** | "Deferred; the agent senses/acts but does not move up-gradient." | **Built and measured (F-08)**: a gradient-directed external-potential force biases the CPM pixel-copy up the sensed shared-glucose gradient — genuine directed motion (~90× the thermal drift), reaching a ~2.1× richer region and out-growing the stationary control (+16% glucose, +13% biomass, +36% volume). Off by default → flagship byte-identical. |
...
## Named next builds — now BUILT and measured (rev-12c)
- **Protocell v2** ✅ — emergent local-mechanism closure achieved (interior precursor confined by a closed membrane; no global observer). EMERGE upgraded from authored to genuinely emergent. Open-system driven version is the one remaining step.
- **M7a cross-feed at 2:1 diffusivity** ✅ — the handoff **fails** at physical diffusivity (consumer displaced); feeding scales monotonically with transport (+14/+45/+102/+203% at 5×/10×/20×/37.5×). The flagship's acceleration is **load-bearing for this geometry**, measured.
- **M7b dividing-population competition** ✅ — robust **dominance** (fast ~9× slow), monoculture-controlled (Tilman R*, not intrinsic death); full exclusion only stochastic (2/5 seeds) → "competitive exclusion" **dropped** for "dominance."
- **M7d steady-state + cadence** ✅ — a source/sink regime reaches a **verified flux steady state** (needs a logistic carrying-capacity — a real design finding); the cadence-halving check found **one pin that moves** (local footprint glucose, 15.7% drift, `mcs_per_update` not interval-scaled) → F-02's local depths re-leveled to cadence-1 readings; the time-integrated claims are cadence-robust.
