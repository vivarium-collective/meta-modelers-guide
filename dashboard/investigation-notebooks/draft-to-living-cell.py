...
# disintegration-metrics
_save_viz('disintegration', 'disintegration-metrics', _render_one('html:viz/disintegration-metrics.html', {'chart': 'html', 'caption': 'Synced metrics for the disintegration run -- area collapses at released_tick == 7, particle count climbs to 68 and holds, over 20-24 ticks.'}, RUNS_DB, STUDY_YAML))
...
# | shed-material-becomes-debris-particles | kind=observable path=particles expr=len(particles) once area reaches 0 | op >= kind acceptance_criterion value 20.0 provenance prior: as the domain resorbs, its vacated footprint pixels are emitted as debris particles that accumulate in the shared store -> expect a substantial, growing particle count. Observed: exactly 68 debris particles shed by tick 16 (8/tick capped, deterministic across repeated runs), staying at 68 for the remainder of the run; the >= 20.0 floor is a loose pin, the accumulation is the a-priori prediction. The 68-vs-56px discrepancy is an unresolved possible double-count pending a mass ledger (tests/test_disintegration_regime.py::test_cell_holds_then_disintegrates_into_scattering_debris) |
...
#
# **Claim.** The paper's §Development and evolution section (Fig 10c-f) holds as a spatial mechanism on the real dividing CPM colony, REUSING study 8's `CpmGrowthDivision` unchanged and adding only a heritable per-cell coupling parameter. DEVELOPMENT: a growing colony on a shared depleting glucose field forms emergent core-vs-rim physiological heterogeneity -- the radial rim-core glucose ratio grows from 1.0 to ~1.44 over 45 ticks (core more depleted than rim, a single-seed reading on the flagship seed 3; a 5-seed band is the named next step), a collective interface not imposed but emerging from shared-field competition. EVOLUTION: a heritable glucose-uptake trait (vmax) shifts its population mean UP under shared-field selection in 4/5 seeds (delta range [-0.116, +0.700], mean +0.221; one seed reverses -- the multi-seed result). A DUAL control points to selection: the no-mutation control stays exactly at the founder value with zero variance (0/5 seeds, no raw material), and the no-selection control mutates (var > 0) but drifts undirected (2/5 up, mean -0.039). The directional up-shift appears only under selection (4/5 vs 2/5), a pattern CONSISTENT WITH selection operating via differential division on the shared field (higher-vmax cells grow/divide more, so their trait proliferates) -- but with n=5 seeds 4/5 vs 2/5 is not statistically significant, so this points to selection as the driver without establishing it; a drift-null at >=20 seeds with a rank test, and re-gating on that ensemble statistic rather than the flagship seed 3, is the named next step. The committed gate currently evaluates the flagship seed 3. The boundaries are stated as findings: the development and evolution halves pull the same field-depletion knob and both saturate within the ~gen 4-6 crowding window; the scope is a toy evo-devo (development = shared-field core/rim heterogeneity, NOT morphogenesis; evolution = one scalar trait under resource selection, NOT genome evolution; facilitated variation -- evolving the ports/couplings themselves -- is a named deferral).
...
# | test | measures | passes if |
# | --- | --- | --- |
# | development-heterogeneity-emerges | kind=observable path=obs.rim_core_ratio expr=last(obs.rim_core_ratio) | op > value 1.2 kind acceptance_criterion provenance prior -- a growing colony on a shared depleting field should form core-vs-rim heterogeneity, so rim_core_ratio should rise above 1.0 (core more depleted than rim); the 1.2 floor is a loose pin on that a-priori direction (not the exact 1.44). Confirmed -- rim_core_ratio grows 1.0 (5 ticks) -> ~1.44 (45 ticks) on the flagship seed 3. SINGLE-SEED (flagship seed 3); a 5-seed band + per-cell scatter is the named next step (Tier C) (tests/test_dev_evo_regime.py::test_development_heterogeneity_emerges, tests/test_cpm_evolution.py::test_rim_core_ratio_shows_heterogeneity) |
# | selection-shifts-mean-trait-up | kind=observable path=obs.mean_vmax expr=last(obs.mean_vmax) | op > value 1.6 kind acceptance_criterion provenance prior -- if the heritable trait feeds each cell's dFBA uptake, selection through differential division should shift the population-mean vmax UP above the founder 1.5; expect mean_vmax > 1.6. Confirmed on the flagship seed 3: mean_vmax 1.500 -> 2.200 (+0.700); multi-seed (seeds 1-5) UP in 4/5, range [-0.116, +0.700], mean +0.221 -- reported as a fraction, not per-seed. The committed gate currently evaluates the flagship seed 3; with n=5 the 4/5-vs-2/5 contrast is not statistically significant, so re-gating on a >=20-seed ensemble statistic is the named next step (Tier C) (tests/test_dev_evo_regime.py::test_evolution_shifts_trait_under_selection, tests/test_cpm_evolution.py::test_selection_shifts_mean_vmax_up, tests/test_cpm_evolution_spike.py::test_selection_shifts_mean_trait_up) |
...
# | no-selection-control-mutates-undirected | kind=observable path=obs.var_vmax expr=last(obs.var_vmax) | op > value 0.0 kind acceptance_criterion provenance prior -- with selection off but mutation on, the trait should still mutate (var_vmax > 0) while conferring no fitness, so the mean should drift undirected rather than shifting up. Confirmed -- selection=false: var_vmax > 0 (still mutates) but the mean drifts undirected -- UP in 2/5 seeds, delta range [-0.270, +0.112], mean -0.039 (vs selection-ON 4/5, +0.221). The committed gate asserts only var_vmax > 0; the 2/5 undirected drift is the reported (ungated) contrast, and with n=5 the 4/5-vs-2/5 gap is not statistically significant (tests/test_dev_evo_regime.py::test_no_selection_control_does_not_directionally_shift) |
