...
# genuinely closed (a real depletion → uptake feedback) in the flagship, while directed control —
# chemotaxis up the sensed gradient — is the flagship's non-motile-by-design other half, now
# BUILT and MEASURED in the `chemotaxis` variant (finding F-08). Mechanistically, the cell and its environment are
...
# - **chemotaxis** — `chemotaxis_strength=50.0`
...
# | chemotaxis-moves-cell-up-gradient-and-helps | kind=observable path=obs.position, obs.local_nutrient, obs.biomass, obs.volume, fields.glucose expr=chemotaxis vs flagship-control COM-x displacement + nutrient/growth deltas over 20 ticks | kind acceptance_criterion op > value 10.0 provenance MEASURED, deterministic fixed-seed run (tests/test_chemotaxis_field.py::test_chemotaxis_moves_cell_up_gradient_and_helps_growth). DIRECTED MOTION — chemotaxis COM x 18.5 → 39.3 (+20.8 px up-gradient) vs flagship control 18.5 → 18.73 (+0.2 px thermal drift), ~90× — pass_if value 10.0 is the up-gradient displacement floor the migrating cell clears (+20.8) and the control does not (+0.2). REACHES RICHER REGION — sensed local glucose 1.95 vs 0.94 (~2.1×). HELPS — net glucose acquired 36.8 vs 31.6 (+16%), biomass 0.418 vs 0.370 (+13%), CPM volume 150 vs 110 px (+36%). Directed up-gradient motion is genuinely achieved and measurably improves nutrient acquisition and growth versus the non-motile flagship on the same field. |
# | chemotaxis-off-is-byte-identical-to-flagship | kind=observable path=obs.biomass, obs.volume, obs.position expr=flag-absent run == flag-explicitly-0.0 run (exact equality of endpoints) | kind regression_pin op approx_equal value 0.0 provenance MEASURED (tests/test_chemotaxis_field.py::test_chemotaxis_off_is_byte_identical_to_flagship) — flag-absent and flag-explicitly-0.0 runs return bit-identical (biomass, volume, COM-x, COM-y); the flagship endpoints are unchanged (biomass ~0.369, CPM volume 110, only thermal up-gradient drift). The chemotaxis term is off-by-default and byte-identical to the flagship. |
...
# | `protocell-autopoietic-v2-open` | `meta_modelers_guide.composites.protocell-autopoietic-v2-open` | 0 | — |
...
# **Composite `meta_modelers_guide.composites.protocell-autopoietic-v2-open`** — `spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open` (a plain, editable dict)

from viva_superpowers.composite_spec import load_spec
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open = load_spec(REPO / 'meta_modelers_guide/composites/protocell-autopoietic-v2-open.composite.json')
describe_spec(spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open)

# === Edit parameters for composite 'protocell-autopoietic-v2-open' ===
# Each line is the spec's CURRENT value — change any, then run the Run cell
# below. The spec is a plain dict, so you may also add or remove keys.

# process 'protocell'  (local:ProtocellV2Open)
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['grid']['nx'] = 64
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['grid']['ny'] = 64
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['D'] = 0.02
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['k_decay'] = 0.01
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['k_prod'] = 0.06
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['thr'] = 0.3
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['dt'] = 1.0
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['Mp'] = 0.1
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['alpha'] = 0.9
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['s_p'] = 0.03
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['k_leak'] = 0.03
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['k_cons'] = 0.03
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['phi_max'] = 1.2
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['f_in'] = 0.01
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['k_wash'] = 0.02
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['k_turn'] = 0.0
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['steps_per_tick'] = 50
spec_meta_modelers_guide_composites_protocell_autopoietic_v2_open['state']['protocell']['config']['seed'] = 1
...
# | v2-open-drive-does-not-sustain | kind=observable path=obs.persists expr=last(obs.persists) | op == value 0.0 kind acceptance_criterion deterministic_pin True provenance prior -- external drive was hypothesised to possibly turn v2''s metastable closure into an indefinite steady self-maintenance; the honest measured result is the opposite. Confirmed -- the canonical drive (f_in=0.01, k_wash=0.02) loses closure at internal step ~591 (vs the undriven ~2451), membrane_mass runs away to ~3092 (seed ~624) as the whole 64x64 domain fills, persists == 0.0 and collapse_tick > 0 by 1000 steps. An ~80-point drive sweep (feed, washout, feed+washout+turnover, saturable export, substrate feed) found NO local drive reaching a bounded steady enclosed_area > 0. The == 0.0 is a deterministic pin (persists is binary, no RNG) -- exact only while the engine build is fixed (tests/test_protocell_v2_open.py::test_composite_open_does_not_sustain, tests/test_protocell_v2_open.py::test_open_drive_destabilises_faster_than_closed) |
# | v2-open-drive-off-recovers-v2 | kind=observable path=obs.persists expr=last(obs.persists) | op == value 1.0 kind regression_pin deterministic_pin True provenance with f_in=k_wash=0 the open composite persists == 1.0, enclosed_area > 100, collapse_tick == -1.0 at 1000 steps -- identical to v2 -- so the ~591-step collapse under drive is caused by the drive alone. The == 1.0 is a deterministic pin (no RNG) (tests/test_protocell_v2_open.py::test_composite_open_off_recovers_v2_persistence, tests/test_protocell_v2_open.py::test_drive_off_reduces_exactly_to_closed_v2) |
# | v2-open-controls-still-load-bearing | kind=observable path=obs.enclosed_area expr=last(obs.enclosed_area) | op == value 0.0 kind acceptance_criterion deterministic_pin True provenance prior -- if closure is genuinely load-bearing, the external drive should not rescue a no-precursor membrane nor a punctured one. Confirmed -- under the canonical drive the s_p=0 knockout loses closure at ~step 188 (enclosed_area -> 0) and a wedge puncture leaves enclosed_area == 0 for the rest of the run (never restored). The == 0.0 is a deterministic pin (no RNG) (tests/test_protocell_v2_open.py::test_open_precursor_knockout_still_collapses, tests/test_protocell_v2_open.py::test_open_puncture_is_not_healed_by_drive) |
