...
| 3 | `cell-cell-coupling` | Cell–cell / *(no own fig; shared env)* | two cell interfaces + shared env + adhesion/signaling ports | **NEW.** Viability *negotiation*: competition (depletion pushes a neighbor out of bounds) vs cross-feeding (cooperation stabilizes both). Pairwise coupling only — biofilm/collective nesting lives in study 9. |
