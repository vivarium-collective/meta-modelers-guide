# meta-modelers-guide + vivarium-workbench — first-run agent guide

> **You are a coding agent (Claude Code / Cursor / …). A first-time user handed
> you this file to get them onto the `meta-modelers-guide` + vivarium-workbench
...
- **This workspace (`meta-modelers-guide`)** — a
  ...
  **typed, independently-wireable processes** you *compose* into whatever
  ...
  `pbg_meta_modelers_guide/` (`core.py` exposes `build_core()`); research state lives
...
```bash
git clone <this repo's URL> meta-modelers-guide
cd meta-modelers-guide
```

**Gate:** `workspace.yaml` and `pbg_meta_modelers_guide/core.py` exist.
...
**Gate:** `.venv/bin/python -c "import pbg_meta_modelers_guide; print('ok')"` prints
...
- **Registry** — the process and type definitions this workspace registers
  (from `pbg_meta_modelers_guide/core.py`'s `build_core()`).
...
- **`ImportError` for `pbg_meta_modelers_guide` in the dashboard** — the editable
  install didn't run or ran into the wrong venv. Re-run `uv pip install -e .`
  and confirm `.venv/bin/python -c "import pbg_meta_modelers_guide"`.
